...
class TestCreateUser:
    ...
    def test_create_user_fail_data_not_complete(self,data):
        response = requests.post(f'{Urls.URL_MAIN_PAGE}{Handlers.Handler.CREATE_USER}', data=data)
        assert response.json()["success"] == False and response.json()["message"] == 'Email, password and name are required fields'
    ...
    def test_create_user_twice_fail(self):
        ...
        response = requests.post(f'{Urls.URL_MAIN_PAGE}{Handlers.Handler.CREATE_USER}', data=data)
        assert response.json()["success"] == False and response.status_code == 403 and response.json()["message"] == 'User already exists'
